...
/// <remarks>
/// <para>
/// The Tag provides metadata to assist in managing otherwise opaque data blocks. It is not tightly
/// bound to the data itself, but rather describes characteristics such as identifiers, storage
/// locations, data formats, or content types. Despite the provided metadata, all inputs should be
/// validated.
/// </para>
/// <para>
/// Unlike string-keyed dictionaries, this uses <see cref="Type"/> as the key, providing compile-time
/// safety and avoiding magic strings. Values are retrieved using <see cref="Get{T}"/> which infers the
/// key from the generic type parameter.
/// </para>
/// <para>
/// Tags are immutable once created. Use <see cref="Create"/> to construct new tags and
/// <see cref="With{T}"/> or <see cref="With"/> to derive new tags from existing ones. All methods
/// return a new <see cref="Tag"/> backed by a <see cref="FrozenDictionary{TKey, TValue}"/> optimised
/// for read-heavy access.
/// </para>
/// </remarks>
...
public record Tag(IReadOnlyDictionary<Type, object> Data)
{
    ...
    /// <summary>Creates a new tag from the specified key-value pairs.</summary>
    ...
    /// <summary>
    /// Returns a new tag that contains all entries from this tag plus the provided items, with the
    /// provided items taking precedence on key conflicts.
    /// </summary>
    ...
    /// <summary>
    /// Returns a new tag that contains all entries from this tag plus one additional entry, inferred
    ...
    /// <summary>
    /// Returns a new tag that contains all entries from this tag except the entry keyed by
    /// <typeparamref name="T"/>. If no such entry exists the original tag is returned unchanged.
    /// </summary>
    /// <typeparam name="T">The type key of the entry to remove.</typeparam>
    /// <returns>A new <see cref="Tag"/> without the entry for <typeparamref name="T"/>, or this tag if none was present.</returns>
    ...
    /// <summary>Retrieves a value from the tag, inferring the key from the generic type.</summary>
    ...
    /// <summary>Attempts to retrieve a value from the tag.</summary>
    /// <typeparam name="T">The type of the value to retrieve, also used as the key.</typeparam>
    ...
    /// <summary>Gets the value associated with the specified key in the tag data.</summary>
